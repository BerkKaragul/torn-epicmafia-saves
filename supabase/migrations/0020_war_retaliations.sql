-- Surface retaliation hits in the war report. Torn's chain reports already
-- carry a per-member retaliation count (chain_contributions.retaliations); we
-- just weren't showing it. It's a BREAKDOWN stat, not an additive one — a
-- retaliation is also counted as a war hit or an outside hit — so it's exposed
-- as its own column rather than folded into another total (which would double
-- count).
...
as $$
  ...
  select i.member_id,
   ...
   order by 3 desc, 8 desc, 10 desc, 2;
$$;
