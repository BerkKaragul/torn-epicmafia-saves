-- war_retals (retals + assists from faction attacks) is authoritative ONLY once
-- a war has been fetched (retals_synced). Until then everything falls back to the
-- chain-derived counts — this also cleans up wars fetched before the assists
-- column existed, whose stale assists=0 rows must not shadow the chain assists.
...
as $$
  ...
  select i.member_id,
         ...
         -- war_retals is authoritative ONLY once fetched (synced); otherwise chain
