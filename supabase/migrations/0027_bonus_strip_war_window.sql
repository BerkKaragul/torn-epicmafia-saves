-- Only strip milestone-bonus respect for milestones the chain reached DURING the
-- war. A chain can keep running after a war ends; milestones hit post-war never
-- counted toward the war score, so they must not be subtracted (that would
-- under-pay). We find when the chain crossed each milestone from chain_polls; if
-- that's unknown (no poll data), we fall back to stripping (the old behaviour),
-- so a milestone is only EXEMPT when we have positive evidence it happened after
-- the war ended.

create index if not exists chain_polls_chain_current
  on chain_polls (torn_chain_id, current, polled_at);
  ...
  bon as (
    select cb.member_id,
     ...
     where cb.chain_count >= 50
       and (p_war is null
            or (c.started_at < win.to_at and coalesce(c.ended_at, now()) > win.from_at))
       -- only milestones reached by war end (present in the war score); unknown
       -- crossing time falls back to stripping
       and (p_war is null
            or coalesce(
                 (select min(cp.polled_at) from chain_polls cp
                   where cp.torn_chain_id = cb.torn_chain_id and cp.current >= cb.chain_count),
                 c.started_at
